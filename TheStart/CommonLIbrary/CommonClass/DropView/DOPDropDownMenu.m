...
- (void)drawRect:(CGRect)rect
{
    ...
    //画一条底部线
    CGContextSetRGBStrokeColor(context, 30/255, 30/255, 40/255, 0.7);//线条颜色
...
@property (nonatomic, assign) NSInteger currentSelectedLeftIndex;  // 当前左侧选中
...
#define kTextColor [UIColor colorWithRed:20/255.0 green:20/255.0 blue:20/255.0 alpha:1]
#define kDetailTextColor [UIColor colorWithRed:136/255.0 green:136/255.0 blue:136/255.0 alpha:1]
#define kSeparatorColor [UIColor colorWithRed:60/255.0 green:60/255.0 blue:60/255.0 alpha:1]
#define kCellBgColor [UIColor colorWithRed:30/255.0 green:30/255.0 blue:40/255.0 alpha:0.7]
#define kTextSelectColor [UIColor colorWithRed:54/255.0 green:126/255.0 blue:228/255.0 alpha:1]
#define bgColor [UIColor colorWithRed:0/255.0 green:0/255.0 blue:0/255.0 alpha:0.8]
...
- (void)setDataSource:(id<DOPDropDownMenuDataSource>)dataSource {
    ...
    self.layer.backgroundColor = [UIColor colorWithRed:0/255 green:0/255  blue:0/255 alpha: 1].CGColor;
    self.backgroundColor = [UIColor colorWithRed:0/255 green:0/255  blue:0/255 alpha:0.6];
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        CALayer *bgLayer = [self createBgLayerWithColor:[UIColor colorWithRed:30/255 green:30/255  blue:40/255 alpha: 0.8] andPosition:bgLayerPosition];
        ...
        CATextLayer *title = [self createTextLayerWithNSString:titleString withColor:self.textSelectedColor andPosition:titlePosition];
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        _leftTableView.layer.borderColor = bgColor.CGColor;
        ...
        _rightTableView.layer.borderColor = bgColor.CGColor;
        ...
        _backGroundView.backgroundColor = [UIColor colorWithRed:0/255.0 green:0/255.0 blue:0/255.0 alpha:0.3];
...
- (CALayer *)createBgLayerWithColor:(UIColor *)color andPosition:(CGPoint)position {
    ...
    layer.backgroundColor = [UIColor colorWithRed:30.0/255 green:30.0/255  blue:40.0/255 alpha: 0.8].CGColor;
...
- (CAShapeLayer *)createIndicatorWithColor:(UIColor *)color andPosition:(CGPoint)point {
    ...
    layer.lineWidth = 0.8;
    layer.fillColor = kTextSelectColor.CGColor;
...
- (void)animateBackGroundView:(UIView *)view show:(BOOL)show complete:(void(^)())complete {
    if (show) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            view.backgroundColor = [UIColor colorWithRed:0/255 green:0/255  blue:0/255 alpha: 0.0];
...
- (void)animateRightTableView:(UITableView *)tableView complete:(void(^)(void))complete {
    [UIView animateWithDuration:0.2 animations:^{
            NSInteger RNum = [self->_rightTableView numberOfRowsInSection:0];
            CGFloat RtableViewHeight = RNum * kTableViewCellHeight > self->_tableViewHeight+1 ? self->_tableViewHeight:RNum*kTableViewCellHeight+1;
            self->_rightTableView.frame = CGRectMake(self.origin.x + self.frame.size.width/2, self.frame.origin.y + self.frame.size.height, self.frame.size.width/2 - kTableViewCellWidth, RtableViewHeight);
     }];
}
...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (show) {
        if (haveItems) {
            _leftTableView.frame = CGRectMake(self.origin.x + kTableViewCellWidth, self.frame.origin.y + self.frame.size.height - kTableViewCellWidth, self.frame.size.width/2 , 0);
            
            _rightTableView.frame = CGRectMake(self.origin.x + self.frame.size.width/2, self.frame.origin.y + self.frame.size.height, self.frame.size.width/2 - kTableViewCellWidth, 0);
            [self.superview addSubview:_leftTableView];
            [self.superview addSubview:_rightTableView];
        } else {
            _leftTableView.frame = CGRectMake(self.origin.x + kTableViewCellWidth, self.frame.origin.y + self.frame.size.height, self.frame.size.width - kTableViewCellWidth*2, 0);
            _rightTableView.frame = CGRectMake(self.origin.x + self.frame.size.width/2, self.frame.origin.y + self.frame.size.height, self.frame.size.width/2 - kTableViewCellWidth, 0);
            [self.superview addSubview:_leftTableView];
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
                ...
                self->_leftTableView.frame = CGRectMake(self.origin.x + kTableViewCellWidth, self.frame.origin.y + self.frame.size.height, self.frame.size.width/2 - kTableViewCellWidth, tableViewHeight);
                ...
                self->_leftTableView.frame = CGRectMake(self.origin.x + kTableViewCellWidth, self.frame.origin.y + self.frame.size.height, self.frame.size.width - kTableViewCellWidth*2, tableViewHeight);
            }
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
                self->_leftTableView.frame = CGRectMake(self.origin.x + kTableViewCellWidth, self.frame.origin.y + self.frame.size.height, self.frame.size.width/2 - kTableViewCellWidth, 0);
                
                self->_rightTableView.frame = CGRectMake(self.origin.x + self.frame.size.width/2, self.frame.origin.y + self.frame.size.height, self.frame.size.width/2 - kTableViewCellWidth, 0);
            } else {
                self->_leftTableView.frame = CGRectMake(self.origin.x + kTableViewCellWidth, self.frame.origin.y + self.frame.size.height, self.frame.size.width - kTableViewCellWidth*2, 0);
            }
            self->_buttomImageView.frame = CGRectMake(self.origin.x, CGRectGetMaxY(self->_leftTableView.frame)-2, self.frame.size.width, kButtomImageViewHeight);
        } completion:^(BOOL finished) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //UIColor *bgColor = [UIColor colorWithRed:0/255 green:0/255  blue:0/255 alpha: 0.6];
    ...
    if (!cell) {
        ...
        bg.backgroundColor = [UIColor colorWithRed:0/255.0 green:0/255.0 blue:0/255.0 alpha:0.4];
    ...
    if (tableView == _leftTableView) {
        ...
        if (indexPath.row == 0 && _currentSelectedLeftIndex == 0){
            cell.textLabel.text = @"不限";
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_leftTableView == tableView) {
        ...
        if (isClickHaveItemValid && _delegate && [_delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
            _currentSelectedLeftIndex = indexPath.row;
            [self animateRightTableView:_leftTableView complete:^{
            }];
